<template>
    <ul class="flex space-x-2 pb-4 rtl:space-x-reverse">
        <li>
            <router-link :to="{ name: 'registration-index' }"
                class="text-primary hover:underline">Inscripciones</router-link>
        </li>
    ...
    <div class="panel p-5">
        ...
        <form class="space-y-5 my-5" @submit.prevent="submitForm()">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <multiselect v-model="selectedParticipant" :options="participants" class="custom-multiselect"
                        :searchable="true" placeholder="Seleccione un participante" selected-label="" select-label=""
                        :label="'name'" :track-by="'id'" deselect-label=""
                ...
                <div>
                    ...
                    <multiselect v-model="selectedTypeParticipant" :options="typeParticipants"
                        class="custom-multiselect" :searchable="true" placeholder="Seleccione una condición"
                        selected-label="" select-label="" :label="'name'" :track-by="'id'" deselect-label=""
            ...
            <div class="ltr:text-right rtl:text-left flex justify-between mt-8">
                ...
                <button type="button" class="btn btn-info ltr:ml-4 rtl:mr-4" @click="handleViewImportParticipants()">
        ...
        <div class="panel px-0 pb-1.5 border-[#e0e6ed] dark:border-[#1b2e4b]">
            <div class="datatable invoice-table">
                <div class="mb-4.5 px-5 flex md:items-center md:flex-row flex-col gap-5">
                    <div class="flex items-center gap-2">
                        <h1 class="text-xl font-semibold">
                            ...
                            <span v-if="isLoadingFetchRegistrationParticipants"
                                class="animate-spin border-2 border-black dark:border-white border-l-transparent rounded-full w-5 h-5 ltr:mr-4 rtl:ml-4 inline-block align-middle">
                            </span>
                        </h1>
                    </div>
                ...
                <vue3-datatable ref="datatable" :loading="isLoadingFetchRegistrationParticipants"
                    :rows="participantsRegistrations" :columns="cols" :totalRows="participantsRegistrations?.length"
                    :sortable="true" :search="search" paginationInfo="Mostrando {0} a {1} de {2} entradas"
                    noDataContent="No hay datos disponibles" skin=""
                    ...
                    <template #actions="data">
                        <div class="flex justify-center gap-x-2">
                            <button type="button" class="btn btn-warning btn-sm"
                                @click="handleViewCertificateParticipant(data.value)">
                                <icon-eye class="w-4 h-4 mr-1" />
                                Certificado
                            </button>
                            <button type="button" class="btn btn-danger btn-sm"
                                @click="showAlertDeleteRegistration(data.value.id)">
        ...
        <!-- Modal Certificado -->
        <ModalCertificateFormView :is-open-modal="isOpenModalCertificate" :registration-id="registrationId"
            :participantName="participantName" @close-modal="handleCloseModalCertificate" />

        <!-- Modal Certificado -->
        <ModalImportParticipantsFormView :is-open-modal="isOpenModalImportParticipants"  @close-modal="handleCloseModalImportParticipants" />
    </div>
</template>
...
import ModalImportParticipantsFormView from './ModalImportParticipantsFormView.vue';
...
// Modal Importación de participantes
const isOpenModalImportParticipants = ref(false);

const handleCloseModalCertificate = () => {
    isOpenModalCertificate.value = false;
};

const handleCloseModalImportParticipants = () => {
    isOpenModalImportParticipants.value = false;
...
const handleViewImportParticipants = () => {
    isOpenModalImportParticipants.value = true;
};
